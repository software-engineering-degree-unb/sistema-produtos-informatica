...
Substitui o script original ``2 - dados.sql``.
Executar depois do ``setup_database.py``.
"""

from app.config.database import get_connection

DEFAULT_PASSWORD_HASH = "$2a$10$z5y3iSLFfTeg/cui.YN29OujBx5bLAbku3QMCyn40uVIPhi1xzJq2"

TIPOS_PRODUTO = [
    "Placa Mãe", "Memória RAM PC", "Memória RAM Notebook", "Processador",
    "Placa de Vídeo", "SSD", "HD", "Gabinete", "Fonte", "Mouse", "Teclado",
    "Monitor", "Cooler", "Notebook", "Headset", "Webcam", "Hub USB",
    "Roteador", "Impressora", "Tablet", "Smartphone",
]
...
USUARIOS_COMUNS = [
    ("João Silva", "12345678901"), ("Maria Santos", "23456789012"),
    ("Pedro Oliveira", "34567890123"), ("Ana Costa", "45678901234"),
    ("Carlos Pereira", "56789012345"), ("Luciana Almeida", "67890123456"),
    ("Roberto Martins", "78901234567"), ("Fernanda Lima", "89012345678"),
    ("Ricardo Souza", "90123456789"), ("Patrícia Ferreira", "01234567890"),
    ("Eduardo Ribeiro", "10293847560"), ("Juliana Gomes", "20394857610"),
    ("Gustavo Santos", "30495867120"), ("Camila Rodrigues", "40596877230"),
    ("Felipe Costa", "50697887340"), ("Amanda Alves", "60798897450"),
    ("Lucas Pereira", "70899807560"), ("Daniela Martins", "80900817670"),
    ("Marcelo Lima", "90011827780"), ("Beatriz Ferreira", "10123836890"),
    ("Rodrigo Carvalho", "11223344556"), ("Mariana Silva", "22334455667"),
    ("Victor Oliveira", "33445566778"), ("Isabela Costa", "44556677889"),
    ("Henrique Santos", "55667788990"),
]

USUARIOS_ADMIN = [
    ("Gerente Vendas", "66778899001", "gerente"),
    ("Supervisor TI", "77889900112", "supervisor"),
]
...
ENDERECOS = [
    ...
    (24, "91330-001", "RS", "Porto Alegre", "Av. Protásio Alves", "2300", None),
    ...
    try:
        with conn.cursor() as cursor:
            for tipo in TIPOS_PRODUTO:
                cursor.execute("INSERT INTO tipoProduto (descricao) VALUES (%s)", (tipo,))

            for produto in PRODUTOS:
                cursor.execute(
                    "INSERT INTO produto (idVisibilidadeProduto, nomeProduto, codigoProduto, idTipoProduto, valorProduto, descricaoProduto) VALUES (%s, %s, %s, %s, %s, %s)",
                    produto,
                )

            for nome, documento in USUARIOS_COMUNS:
                cursor.execute(
                    "INSERT INTO usuario (nome, documento, idSituacaoUsuario) VALUES (%s, %s, 1)",
                    (nome, documento),
                )

            admin_ids = []
            for nome, documento, login in USUARIOS_ADMIN:
                cursor.execute(
                    "INSERT INTO usuario (nome, documento, idSituacaoUsuario) VALUES (%s, %s, 1)",
                    (nome, documento),
                )
            ...
            cursor.execute(
                "SELECT idUsuario FROM usuario WHERE idUsuario > 2 AND idUsuario <= 27 ORDER BY idUsuario"
            )
            usuarios_comuns_ids = [row["idUsuario"] for row in cursor.fetchall()]

            for id_usuario in usuarios_comuns_ids:
                cursor.execute(
                    "INSERT INTO login (idUsuario, login, senha, idTipoLogin, idSituacaoUsuario) VALUES (%s, %s, %s, 1, 1)",
                    (id_usuario, f"user{id_usuario}", DEFAULT_PASSWORD_HASH),
                )

            for id_usuario, login in zip(admin_ids, [u[2] for u in USUARIOS_ADMIN]):
                cursor.execute(
                    "INSERT INTO login (idUsuario, login, senha, idTipoLogin, idSituacaoUsuario) VALUES (%s, %s, %s, 2, 1)",
                    (id_usuario, login, DEFAULT_PASSWORD_HASH),
                )

            for endereco in ENDERECOS:
                cursor.execute(
                    "INSERT INTO endereco (idUsuario, cep, uf, municipio, rua, numero, complemento) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                    endereco,
                )
